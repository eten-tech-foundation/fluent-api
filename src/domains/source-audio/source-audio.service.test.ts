...
import { getChapterSourceAudio, matchAquiferBible } from './source-audio.service';
...
describe('matchAquiferBible', () => {
  ...
  it('falls back to language default', () => {
    const candidates: AquiferBible[] = [
...
describe('getChapterSourceAudio', () => {
  ...
  it('returns DBL tracks when available', async () => {
    ...
    if (result.ok) {
      ...
      expect(result.data.verseTimestamps?.[0]?.verse).toBe(1);
    }
