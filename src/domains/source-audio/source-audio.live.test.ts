...
import { bibles, projects } from '@/db/schema';
import { AUDIO_DEMO_SEED } from '@/db/seeds/audio-demo';
import { getReferencePlayback } from '@/domains/playback-audio/playback-audio.service';
import { playbackAudioResponseSchema as sourceAudioResponseSchema } from '@/domains/playback-audio/playback-audio.types';
import env from '@/env';
...
// windowless branch as an exact reference without importing language or Bible rows.
...
describe('source-audio resolver against live Aquifer', () => {
  ...
  it('the seeded BSB John 3 returns 36 complete windows through the seeded project route, with one catalogue call', async () => {
    ...
    const endpoint = `/projects/${project.id}/playback-audio/JHN/3?languageCode=eng&bibleId=${bible.id}`;
  ...
  it('the IRV Hindi John 1 chapter is measured windowless and the resolver labels it false', async () => {
    ...
    const result = await getReferencePlayback({
      identity: { provider: 'aquifer', externalId: '2' },
      languageCode: 'hin',
      bookCode: 'JHN',
      chapter: 1,
    });
    ...
    expect(audio.verseTimestamps ?? []).toEqual([]);
    ...
    const raw: Awaited<ReturnType<typeof aquifer.getBibleText>> = await text.mock.results[0]!.value;
