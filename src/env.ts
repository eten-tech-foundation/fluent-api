...
const EnvBaseSchema = z.object({
  ...
  // Source TTS — lets a drafter hear the source text read aloud while drafting.
  // The audio is generated by fluent-ai, so this is AI-dependent and takes the
  // same contract as the two flags above: unset derives from AI wiring
  // (aiIsWired), which is safe-off when fluent-ai isn't reachable.
  //
  ...
  EN_FEATURE_SOURCE_TTS: envBool().optional(),
});
