...
| **Project-Pinned** <br> `(orgId: O, projectId: P)` | `Project Translator` <br> `Project Observer` | The grant is strictly confined. A Project Translator only has update access to chapters/verses assigned to them *inside* project `P` of organization `O`. |
...
*   If the user holds the permission in *at least one* grant (e.g. a Translator having `project:view` on Project A), they pass Layer 2.
